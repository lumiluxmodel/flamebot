...
async function updateBio(useAI = true, customBio = null) {
  console.log(
    `\n${colors.bright}📝 Updating Bio ${
      useAI ? "(AI Generated)" : "(Custom Text)"
    }${colors.reset}`
  );
  ...
  try {
    ...
    const bioStatus =
      response.data.data.result.results[0].fields.bio_information?.status ||
      "Unknown";
    if (bioStatus === "Successful") {
      console.log(
        `${colors.green}   ✅ Bio update status: ${bioStatus}${colors.reset}`
      );
    } else {
      console.log(
        `${colors.red}   ❌ Bio update status: ${bioStatus}${colors.reset}`
      );
    }
    ...
    console.error(
      `${colors.red}❌ Failed to update bio:${colors.reset}`,
...
async function updatePrompt(useAI = true, customPrompt = null) {
  ...
  try {
    ...
    console.log(
      `${colors.green}✅ Prompt updated successfully!${colors.reset}`
    );
    console.log(`   Task ID: ${colors.cyan}${taskId}${colors.reset}`);
    console.log(`   Status: ${response.data.data.result.status}`);
    console.log(`   Progress: ${response.data.data.result.progress}`);
    ...
    if (obfuscatedText) {
      console.log(
        `   Obfuscated Preview: ${obfuscatedText.substring(0, 60)}...`
      );
    }

    // Verificar que se aplicó correctamente
    const promptStatus =
      response.data.data.result.results[0].fields.prompts?.status || "Unknown";
    if (promptStatus === "Successful") {
      console.log(
        `${colors.green}   ✅ Prompt update status: ${promptStatus}${colors.reset}`
      );
    } else {
      console.log(
        `${colors.red}   ❌ Prompt update status: ${promptStatus}${colors.reset}`
      );
    }
    ...
    console.error(
      `${colors.red}❌ Failed to update prompt:${colors.reset}`,
      error.response?.data || error.message
    );
    return { success: false, error: error.message };
  }
...
async function verifyUpdate(taskId, type = "bio") {
  ...
  try {
    const response = await axios.get(
      `${API_BASE_URL}/${type}/status/${taskId}`
    );

    console.log(
      `${colors.green}✅ ${type.toUpperCase()} update verified successfully!${
        colors.reset
      }`
    );
    ...
    console.error(
      `${colors.red}❌ Failed to verify ${type} update:${colors.reset}`,
      error.response?.data || error.message
    );
    return { success: false, error: error.message };
  }
...
╚═══════════════════════════════════════╝${colors.reset}
  ...
  if (!serverRunning) {
    ...
    console.log(
      `   Run: ${colors.cyan}npm run dev${colors.reset} or ${colors.cyan}npm start${colors.reset}`
    );
    process.exit(1);
  }
  ...
  if (!process.env.OPENAI_API_KEY) {
    console.log(
      `${colors.yellow}⚠️ Warning: OPENAI_API_KEY not found - AI generation tests may fail${colors.reset}`
    );
  }

  console.log(`${colors.green}✅ Environment OK${colors.reset}`);
  console.log(
    `${colors.cyan}📝 Testing with Account ID: ${TEST_ACCOUNT_ID}${colors.reset}`
  );
  console.log(
    `${colors.cyan}🎯 Model: ${TEST_MODEL} | Channel: ${TEST_CHANNEL}${colors.reset}\n`
  );
  ...
  console.log(
    `   ${colors.cyan}•${colors.reset} Bio: AI-generated with OnlyFans conversion strategy`
  );
  console.log(
    `   ${colors.cyan}•${colors.reset} Prompt: AI-generated with obfuscation for ${TEST_MODEL}/${TEST_CHANNEL}`
  );
  console.log(
    `   ${colors.cyan}•${colors.reset} Both updates completed successfully`
  );
  console.log(`\n${colors.bright}Next Steps:${colors.reset}`);
  console.log(
    `   1. Check Flamebot dashboard to see the updated bio and prompt`
  );
...
async function testCustomContent() {
  const customBio =
    "hey babe, just living my best life 💕 swipe right if you can handle this energy and want some fun";
...
switch (testType) {
  ...
  case "verify":
    ...
    if (taskId) {
      ...
      console.log(
        `${colors.red}❌ Please provide task ID: npm run test:bio-prompt verify TASK_ID [bio|prompt]${colors.reset}`
      );
    }
    break;
  default:
