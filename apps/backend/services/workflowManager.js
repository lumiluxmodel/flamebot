...
class WorkflowManager {
    ...
    async startAccountAutomation(accountId, accountData, workflowType = 'default') {
        ...
        try {
            ...
            const result = await this.executor.startExecution(accountId, accountData, workflowType);
    ...
    async stopAccountAutomation(accountId) {
        try {
            const result = await this.executor.stopExecution(accountId);
    ...
    setupEventListeners() {
        ...
        this.executor.on('execution:started', (data) => {
            console.log(`📊 Workflow Event: Started - ${data.accountId} (${data.executionId})`);
        });
        ...
        this.executor.on('execution:step_completed', (data) => {
            console.log(`📊 Workflow Step Completed: ${data.stepId} for ${data.accountId}`);
        });

        this.executor.on('execution:step_failed', (data) => {
            console.error(`📊 Workflow Step Failed: ${data.stepId} for ${data.accountId}`);
            console.error(`   Error: ${data.error}`);
        });
