    ...
    <div>
      ...
      <div className="cyber-card p-8 md:p-16 mb-8 md:mb-12 relative animate-fade-in">
        <div className="text-center">
          <ClientOnlyIcon>
            <AlertCircle className="w-12 h-12 md:w-16 md:h-16 mx-auto mb-6 text-zinc-300 dark:text-zinc-800 animate-pulse" />
          </ClientOnlyIcon>
          <div className="text-lg text-zinc-600 mb-2">
            {alerts.length === 0 ? 'NO ACTIVE ALERTS' : `${alerts.length} ACTIVE ALERTS`}
          </div>
          <div className="text-[11px] text-zinc-600 dark:text-zinc-700">
            {alerts.length === 0 ? 'System operating within normal parameters' : 'Check alert details below'}
          </div>
          <div className="mt-4 text-[10px] text-yellow-600 dark:text-yellow-500">
              ...
              <div key={i} className="cyber-card p-4 flex justify-between items-center animate-fade-in" style={{ animationDelay: `${i * 0.1}s` }}>
                <div>
                  <div className={`text-sm font-mono ${
                    ...
                    alert.severity === 'warning' ? 'text-yellow-600 dark:text-yellow-500' : 'text-zinc-600 dark:text-zinc-500'
                  }`}>
                    {alert.message}
                  </div>
                  <div className="text-[10px] text-zinc-600 dark:text-zinc-500 mt-1">
            ...
            {Object.entries(alertCounts).map(([severity, count]) => {
              ...
              return (
                <div key={severity} className="flex items-center gap-3">
                  ...
                  <div className="flex-1 bg-zinc-200 dark:bg-zinc-900 h-2 rounded-full overflow-hidden">
                    <div 
                      className={`h-full ${color} rounded-full transition-all duration-1000`}
                      style={{ width: `${percentage}%` }}
                    />
                  </div>
                  <div className="text-[11px] text-zinc-600 dark:text-zinc-500 w-12 text-right">{count}</div>
                </div>
              );
            })}
          </div>
        ...
        <div className="cyber-card p-6">
          ...
          <div className="space-y-3 text-sm">
            <div className="flex justify-between">
              <span className="text-zinc-600 dark:text-zinc-500">Total Alerts</span>
              <span className="text-zinc-900 dark:text-white">{alerts.length}</span>
            </div>
            <div className="flex justify-between">
              <span className="text-zinc-600 dark:text-zinc-500">Unacknowledged</span>
              <span className="text-yellow-600 dark:text-yellow-500">{alerts.filter(a => !('acknowledged' in a) || !a.acknowledged).length}</span>
            </div>
            <div className="flex justify-between">
              <span className="text-zinc-600 dark:text-zinc-500">Most Recent</span>
              <span className="text-zinc-600 dark:text-zinc-400">
                {alerts.length > 0 ? new Date(alerts[0].timestamp).toLocaleTimeString() : 'None'}
              </span>
            </div>
            <div className="flex justify-between">
              <span className="text-zinc-600 dark:text-zinc-500">System Health</span>
              <span className={alerts.length === 0 ? 'text-emerald-600 dark:text-emerald-500' : 'text-yellow-600 dark:text-yellow-500'}>
