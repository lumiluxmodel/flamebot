...
class WorkflowUI {
    ...
    setupEventListeners() {
        ...
        if (pauseBtn) pauseBtn.addEventListener('click', () => this.pauseAllWorkflows());
        if (resumeBtn) resumeBtn.addEventListener('click', () => this.resumeAllWorkflows());
        ...
        if (refreshWorkflows) refreshWorkflows.addEventListener('click', () => this.loadActiveWorkflows());
        if (bulkStop) bulkStop.addEventListener('click', () => this.bulkStopWorkflows());
        if (viewAllAlerts) viewAllAlerts.addEventListener('click', () => this.switchTab('monitoring'));
        if (refreshActivity) refreshActivity.addEventListener('click', () => this.loadRecentActivity());
        if (acknowledgeAll) acknowledgeAll.addEventListener('click', () => this.acknowledgeAllAlerts());
        if (filterCritical) filterCritical.addEventListener('click', () => this.filterCriticalAlerts());
    ...
    async bulkStopWorkflows() {
        ...
        if (!confirm(`Stop ${this.selectedWorkflows.size} selected workflows?`)) return;
        ...
        this.showNotification(`Bulk stop completed: ${results.successful} successful, ${results.failed} failed`, 'info');
    ...
    async acknowledgeAllAlerts() {
        try {
            const alertsResponse = await fetch(`${this.API_BASE}/monitoring/alerts?unacknowledged=true`);
            ...
            this.showNotification(`Acknowledged ${alertsData.data.alerts.length} alerts`, 'success');
    ...
    switchTab(tabId) {
        ...
        try {
            ...
            console.log(`Processing ${tabButtons.length} buttons and ${tabContents.length} contents`);
    ...
    async loadTabData(tabId) {
        try {
            ...
            this.showNotification(`Failed to load ${tabId} data: ${error.message}`, 'error');
        }
    }
    ...
    updateSystemOverview(data) {
        ...
        if (healthElement) {
            ...
            healthElement.className = `card-status ${isHealthy ? 'healthy' : 'warning'}`;
        }

        // Update metrics
        this.updateElementText('active-workflows-count', data.executor.activeExecutions);
        this.updateElementText('total-executions', data.executor.totalExecutions);
        this.updateElementText('success-rate', `${data.executor.successRate.toFixed(1)}%`);
        this.updateElementText('avg-duration', this.formatDuration(data.executor.averageExecutionTime));

        // Update status bar
        this.updateElementText('workflow-count', `${data.executor.activeExecutions} Active Workflows`);
        this.updateElementText('alert-count', `${data.health.unacknowledgedAlerts} Alerts`);
    ...
    updateWorkflowDistribution(workflowData) {
        ...
        if (total === 0) {
            container.innerHTML = '<div class="empty-state-message">No active workflows</div>';
            return;
        }
    ...
    updateSystemPerformance(data) {
        ...
        this.updateElementText('memory-usage', `${Math.round(process?.memoryUsage?.()?.heapUsed / 1024 / 1024 || 0)}MB`);
    ...
    updateRecentAlerts(alerts) {
        ...
        if (!alerts || alerts.length === 0) {
            container.innerHTML = '<div class="empty-state-message">No recent alerts</div>';
            return;
        }
    ...
    async loadActiveWorkflows() {
        try {
            ...
            const params = new URLSearchParams({
                ...
                ...(this.filters.workflowType && { workflowType: this.filters.workflowType }),
    ...
    createWorkflowCard(workflow) {
        ...
        return `
            <div class="workflow-card ${isSelected ? 'selected' : ''}" data-account-id="${workflow.accountId}">
                <div class="workflow-status ${workflow.status}"></div>
                <div class="workflow-card-header">
                    <div class="workflow-account-id">${workflow.accountId}</div>
                    <div class="workflow-type-badge ${workflow.workflowType}">${workflow.workflowType}</div>
                </div>
                <div class="workflow-card-body">
                    ...
                    <div class="workflow-info">
                        <div class="workflow-info-item">
                            ...
                            <div class="workflow-info-value">${this.formatTimeAgo(workflow.startedAt)}</div>
                        </div>
                        <div class="workflow-info-item">
                            <div class="workflow-info-label">Elapsed</div>
                            <div class="workflow-info-value">${this.formatDuration(workflow.timeElapsed)}</div>
                        </div>
                        <div class="workflow-info-item">
                            <div class="workflow-info-label">Steps</div>
                            <div class="workflow-info-value">${workflow.currentStep}/${workflow.totalSteps}</div>
                        </div>
                        <div class="workflow-info-item">
                            <div class="workflow-info-label">Status</div>
                            <div class="workflow-info-value">${workflow.status}</div>
                        </div>
                    </div>
                    ...
                    ${workflow.nextStep ? `
                        <div class="workflow-current-step">
                            ...
                            <div class="workflow-step-description">${workflow.nextStep.description || workflow.nextStep.action}</div>
                        </div>
                    ` : ''}
    ...
    updateSystemHealth(data) {
        ...
        const components = [
            { name: 'Workflow Executor', healthy: data.systemStatus.workflowExecutor },
        ...
        healthComponents.innerHTML = components.map(comp => `
            <div class="health-component">
                <span class="component-name">${comp.name}</span>
                <div class="component-status">
                    <div class="component-indicator ${comp.healthy ? 'healthy' : 'error'}" 
                         style="background-color: ${comp.healthy ? 'var(--terminal-green)' : 'var(--terminal-red)'}"></div>
                    <span class="component-text ${comp.healthy ? 'text-success' : 'text-error'}">
                ...
                <tbody>
                    ${alerts.map(alert => `
                        <tr>
                            ...
                            <td>
                                <button class="workflow-action-btn" onclick="workflowUI.acknowledgeAlert('${alert.id}')">
                                    ✓ Ack
                                </button>
                            </td>
                        </tr>
            ...
            <div class="definition-card">
                ...
                <div class="definition-body">
                    <div class="definition-steps">
                        ${def.steps.map((step, index) => `
                            <div class="definition-step">
                                ...
                                <div class="step-delay">${this.formatDuration(step.delay)}</div>
                            </div>
                        `).join('')}
                    </div>
                    ...
                    <div class="metric-row">
                        ...
                        <span class="metric-value">${this.formatDuration(def.estimatedDuration)}</span>
                    </div>
                </div>
            </div>
        `).join('');
    ...
    async pauseAllWorkflows() {
        ...
        try {
            ...
            this.showNotification(`Failed to pause workflows: ${error.message}`, 'error');
        }
    }
    ...
    async resumeAllWorkflows() {
        ...
        try {
            ...
            this.showNotification(`Failed to resume workflows: ${error.message}`, 'error');
        }
    }

    async acknowledgeAlert(alertId) {
        try {
            const response = await fetch(`${this.API_BASE}/monitoring/alerts/${alertId}/acknowledge`, {
                method: 'POST'
            });
            ...
            this.showNotification(`Failed to acknowledge alert: ${error.message}`, 'error');
        }
    }
            ...
            <div class="workflow-detail-grid">
                <div class="detail-section">
                    ...
                    <div class="detail-item">
                        ...
                        <span class="detail-value">${workflow.workflowType}</span>
                    </div>
                    <div class="detail-item">
                ...
                <div class="detail-section">
                    ...
                    <div class="execution-log">
                        ${workflow.executionLog.map(log => `
                            <div class="log-entry ${log.success ? 'success' : 'error'}">
                                <div class="log-step">${log.stepId}</div>
                                <div class="log-result">${log.success ? '✓' : '✗'} ${log.result?.message || log.error}</div>
                                <div class="log-time">${this.formatTimeAgo(log.timestamp)}</div>
                            </div>
                        `).join('')}
                    </div>
                </div>
    ...
    showLoading(containerId) {
        ...
        if (container) {
            const loader = container.querySelector('.loading') || document.getElementById('main-loader');
